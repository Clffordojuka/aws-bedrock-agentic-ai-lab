...
def filter_messages(messages: list):
    ...
    for msg in messages:
        ...
        text_entries = [
            entry for entry in content if "text" in entry and entry["text"].strip()
        ]
        if text_entries:
    ...
    greetings = {
        ...
        10: {
            ...
            "specials": [
                ...
                "• Ghostly White Mocha",
                "• Vampire's Blood (Red Velvet Latte)",
            ],
        },
        11: {
            "season": "Thanksgiving",
            ...
            "specials": [
                "• Grateful Pumpkin Latte",
                "• Turkey Day Spice Coffee",
                "• Cranberry Orange Scone with Coffee",
                "• Thankful Chai Latte",
            ],
        },
    }
...
@tool(
    ...
    description=(
        "A tool for checking order status, placing new orders "
...
@tool(
    ...
    description=(
        "A tool for checking price of an item, calculating total cost "
...
@tool(
    ...
    description=(
        "A tool to answer questions about current deals "
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            span_cm = (
                TRACER.start_as_current_span("user_prompt") if TRACER else nullcontext()
            )
            with span_cm as span:
                if span:
                    span.set_attribute(
                        "aws.log.group.names", "barista_supervisor-agent-logs"
                    )
                ...
                try:
                    ...
                    ERROR_MESSAGE = str(e)
                    if (
                        "guardrail" in ERROR_MESSAGE.lower()
                        ...
                        strands_logger.error(
                            "Error processing request: %s", ERROR_MESSAGE
                        )
                    continue
                ...
                for message in reversed(filter_messages(orchestrator.messages)):
                    if message["role"] == "assistant":
                        ...
                        strands_logger.info(
                            "Assistant response: %s", message["content"][-1]["text"]
                        )
                        break
