...
def check_seasonal_availability(item_name: str) -> str:
    ...
    try:
        ...
        for item in menu_data:
            ...
            if (
                base_item_name == menu_item_name
                or base_item_name == item_id
                or base_item_name.replace("_", " ") == menu_item_name
                or menu_item_name.replace(" ", "_") == base_item_name
            ):
                # Check if it's a seasonal item
                if item.get("attributes_json", {}).get("seasonal", False):
                    season_months = item.get("attributes_json", {}).get(
                        "season_months", []
                    )
                    ...
                    return (
                        f"{item['name']} is a seasonal item and is not currently available. "
