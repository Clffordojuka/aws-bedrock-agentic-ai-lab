...
@tool(
    name="get_recent_order", description="A tool to retrieve data about existing orders"
)
...
def validate_seasonal_items(items: list) -> dict:
    ...
    try:
                ...
                if menu_item["name"].lower() == item_name:
                    ...
                    if attrs.get("seasonal", False):
                        ...
                        if current_month not in season_months:
                            next_months = [
                                m for m in season_months if m > current_month
                            ]
                            next_month = (
                                min(next_months) if next_months else min(season_months)
                            )
        ...
        return {
            "valid": False,
            "message": f"Unable to validate seasonal items: {str(e)}",
        }
...
def process_order(order_details: str) -> dict:
    ...
    try:
        ...
        name_match = re.search(
            r"(?:for|customer:?)\s+([A-Za-z]+)", order_details, re.IGNORECASE
        )
        customer_name = (
            name_match.group(1) if name_match else f"customer_{int(time.time())}"
        )
        ...
        return {
            "message": f"Error processing order: {str(e)}. Please provide order details clearly."
        }
    ...
    try:
        ...
        if not validation.get("valid", False):
            ...
            if unavailable:
                ...
                return {
                    "message": (
                        f"Sorry, these seasonal items are not currently available: "
        ...
        return {
            "message": f"Error placing order: {str(e)}. Please try again or contact support."
        }
