...
# === Tools ===
@tool(
    name="payments_kb_retrieve",
    description="A tool to retrieve data from payments knowledge base",
)
...
@tool(
    ...
    description='Calculate total cost based on business pricing rules. Expected input format:items: [{"type": "latte|hot_chocolate|americano|bagel|donut", "size": "small|medium|large", "customizations": ["extra_shot", "oat_milk"]}',
)
    ...
    try:
        ...
        for item in items:
            ...
            # Unknown items - return error instead of fallback pricing
                ...
                return {
                    "error": f"Item '{item['type']}' is not available on the menu. Please check with the menu agent for available items."
                }
